...
def save_routing_to_buffer_v2(
    ...
    if debug_mode and position_ids is not None:
        logger.info(f"Hack routing ids!!")
...
class RoutedExpertsCapturer:
    ...
    def prepare_pending_save(self, num_tokens: int, slot_mapping_gpu: paddle.Tensor, position_ids_gpu: paddle.Tensor = None):
        """
    ...
    def flush_pending_save(self):
        ...
        if self.debug_mode and self.cpu_position_ids_buf is not None:
            ...
            if not np.array_equal(data, expected_routing):
                ...
                logger.error(f"[R3 Debug] flush mismatch! num_tokens={num_tokens}, mismatched_tokens={len(mismatched_token_indices)}")
                ...
                if slot_cpu_slice.min() < 0:
                    ...
                    error_parts.append(f"  token |slot_staging | slot_pinned | slot_cpu    | position_id | data[0,0]")
                ...
                if max_slot >= self.max_num_kv_tokens:
                    ...
                    error_parts = [f"[R3 Debug] Slot indices out of range: num_tokens={num_tokens}, "
                    ...
                    error_parts.append(f"  token |slot    | position_id | data[0,0]")
                    error_parts.append("  " + "-" * 50)
