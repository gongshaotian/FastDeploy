...
class GPUModelRunner(ModelRunnerBase):
    ...
    def _compute_position_ids_and_slot_mapping(self, total_token_num) -> None:
        ...
        if self.routing_replay_manager.debug_mode:
            ...
            for i in range(total_token_num):
                logger.info(
                    f"  {i:4d} | {int(self.forward_meta.position_ids[i]):8d} | {int(self.forward_meta.slot_mapping[i]):7d}"
                )
    ...
    def _dummy_run(
        ...
        while True:
            ...
            self._compute_position_ids_and_slot_mapping(total_token_num=token_num)
