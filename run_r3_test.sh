...
python -m fastdeploy.entrypoints.openai.api_server --config ${config_yaml} --model ${model_path} \
    --tensor-parallel-size 1 --max-model-len 32768 --max-num-seqs 64 \
    ...
    --routing-replay-config '{"enable_routing_replay":true, "routing_store_type":"rdma", "local_store_dir":"./routing_replay_output", "use_fused_put":true, "rdma_store_server":"redis://10.95.239.155:6379"}' \
