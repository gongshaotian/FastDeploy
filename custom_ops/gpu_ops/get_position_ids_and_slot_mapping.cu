...
#pragma unroll
  for (int i = 0; i < token_num_this_batch; i++) {
    ...
    slot_mapping[idx] = static_cast<int64_t>(static_cast<int64_t>(block_id) * block_size + block_offset);
  }
}

void GetPositionIdsAndSlotMapping(
  ...
  GetPositionIdsAndSlotMappingKernel<<<1, bsz, 0, seq_lens_this_time.stream()>>>(
...
PD_BUILD_STATIC_OP(get_position_ids_and_slot_mapping)
    ...
    .SetInplaceMap({{"position_ids", "position_ids_out"}, {"slot_mapping", "slot_mapping_out"}})
